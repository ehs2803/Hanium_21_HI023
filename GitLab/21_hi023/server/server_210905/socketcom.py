...
class Socket():
    ...
    def cleanAlgorithm(self):
        ...
        #####################################################
        ''' 
        five_min_dat가 데이터 베이스에 저장되는 거면 이게 과연 필요할가?
        그리고 이 방식은 조금 변경 필요스
    ...
    def save_fivemin(self):
        ...
        today = datetime.datetime.today().weekday() # 오늘의 요일 0~4 평일 5,6 주밀
        formatted_data = now.strftime('%Y-%m-%d')  # 현재 시간 문자열 포맷팅

        for i, v in self.CAMlist:
            fivemin_json = json.dumps(self.fiveMin_data_dict[i])
            lock.acquire()
            self.cursor.execute('insert into fivemindata values(%s,%s,%s,%s)', (v, formatted_data, today ,fivemin_json))
            self.db.commit()
            lock.release()
    ...
    def make_fiveMin(self):
        ...
        sql_sel = "SELECT human FROM humandata where id = %s and time between %s and %s"
