...
class Socket():
    ...
    def get_document(self):
        self.cursor.execute("SELECT name, time, way FROM test_table3 order by time desc")
    ...
    # 미완
    def algorithms(x):
        # 살균 알고리즘은 마치 sin, cos함수마냥 구동하는 원리입니다.
        # 즉 들어오는 X의 데이터는 사람이 있으면 1 없으면 0이게 되는 데
        # 각 데이터의 누적합을 통해서 sin cos인 마냥 파형을 이룬 데이터를 갖게 되고
        # 파형의 최대값 최소값 지점이 살균의 기준점이 되게 합니다~
        length = len(x)
        data = []  # data를 받을 공간
        res = 0  # 누적합에 도움을 줄 변수!
        criteria = 0
        ...
        for i in range(1, length):
            # print(x['human'][i])
            if x['human'][i] == 0:  # 사람이 없을 때~
                if before == x['human'][i]:  # 전과 비교 동일
                    res -= 1
                else:  # 전에 있다가 이제 없는 거 나름 살균 포인트
                    if res > 6:  # 나름 30분 넘게 있던거
                        print("up", x['time'][i], i, res)
                        data.append(x['time'][i])
                        res = 0
            else:  # 사람이 있을 때~
                if before == x['human'][i]:  # 전과 비교 동일
                    res += 1
                    ...
                    print("dd", x['time'][i - 1], i, res)
                    data.append(x['time'][i - 1])
                    res = 0
            before = x['human'][i]
    ...
    def save_cleantime(self):
        ...
        for i1,v  in enumerate(self.cleantime):
            ...
            for j in v:
                ...
                lock.acquire()
                self.cursor.execute('insert into test_table3 values(%s,%s,%s,%s)',(id, name, str, "AI"))
                self.db.commit()
                lock.release()
    ...
    def command_clean(self, id):
        ...
        formatted_data = now.strftime('%Y-%m-%d %H:%M')  # 현재 시간 문자열 포맷팅
        lock.acquire()
        self.cursor.execute('insert into test_table3 values(%s,%s,%s,%s)', (id, name, formatted_data,"user"))  # 데이터 삽입
        self.db.commit()
        lock.release()
